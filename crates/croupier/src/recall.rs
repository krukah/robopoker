...
pub trait Recall {
    ...
    ///
    /// Maps each `Action` onto an `Edge` via the global [`fulcrum::translation`].
    /// All current [`fulcrum::Translation`] variants (`Snap`, `Harmonic`,
    ...
    fn history(&self) -> Vec<Edge> {
        let translation = fulcrum::translation();
