...
impl Size {
    ...
    ///
    /// Nlhe-specific adapter over [`calque`]: dispatches to the
        ...
        R: rand::Rng + ?Sized,
    {
        ...
        match Self::grid(raise.street(), raise.depth()) {
            ...
            Some(Grid::Opening(bbs)) => Self::translate_axis::<BB, _>(
                raise.chips(),
                raise.chips() as f64 / fulcrum::B_BLIND as f64,
    ...
    /// `None` if depth exceeds [`fulcrum::MAX_RAISE_REPEATS`].
    pub fn grid(street: Street, depth: usize) -> Option<Grid> {
        if depth > fulcrum::MAX_RAISE_REPEATS {
            None
        } else {
            match fulcrum::regime() {
                fulcrum::Regime::Pluribus => Some(Self::pluribus_grid(street, depth)),
                fulcrum::Regime::Slumbot => Some(Grid::Postflop(SLUMBOT_INDICES)),
            }
        }
    }

    /// Returns the raw RAISES-index slice for a given cell from the
    /// `PLURIBUS_INDICES` table-of-truth in `fulcrum`.
    ///
    ...
    pub fn raises(street: Street, depth: usize) -> Vec<Self> {
        ...
        match fulcrum::regime() {
            ...
            fulcrum::Regime::Slumbot => SLUMBOT_INDICES
                .iter()
                .map(|&i| {
                    let (n, d) = RAISES[i];
                    Self::SPR(n, d)
...
mod tests {
    ...
    fn into_chips_bbs() {
        let pot = 100;
        assert_eq!(Size::BBs(2).into_chips(pot), 2 * fulcrum::B_BLIND);
        assert_eq!(Size::BBs(3).into_chips(pot), 3 * fulcrum::B_BLIND);
        assert_eq!(Size::BBs(5).into_chips(pot), 5 * fulcrum::B_BLIND);
    }
