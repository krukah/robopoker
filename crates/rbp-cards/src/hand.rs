...
impl Hand {
    /// The empty hand containing no cards.
    pub fn empty() -> Self {
        Self(0)
    }
    /// Union of two hands. Allows overlap (idempotent).
    ...
    /// Highest rank in the hand, if non-empty.
    ...
    /// Removes a card from the hand in place.
    pub fn remove(&mut self, card: Card) {
        let card = u8::from(card);
        let mask = !(1 << card);
        self.0 &= mask;
    }
    /// Tests whether the hand contains a specific card.
    pub fn contains(&self, card: &Card) -> bool {
        self.0 & (1 << u8::from(*card)) != 0
    }
    /// Tests whether two hands share any cards.
    pub fn overlaps(a: &Hand, b: &Hand) -> bool {
        a.0 & b.0 != 0
    }
    ...
    /// Bitmask for all valid card positions (52 bits for standard, 36 for short deck).
