...
//!
//! `let _guard = vitals::init();` installs a `tracing` subscriber, routes
//! `log::*` through it via the `tracing-log` shim, configures the OTLP metrics
//! exporter, and pre-registers the workspace's metric handles under `metrics::*`.
//!
...
///
/// Info-by-default: keeps lifecycle events while dropping the per-event debug
/// chatter that drove 99% of log ingestion cost. Hot-path logs are `debug!`/
/// `trace!` and suppressed here. Override with `RUST_LOG=debug` or
/// `RUST_LOG=spar=debug`.
...
/// OpenTelemetry default of `[0, 5, …, 10000, +Inf]`. Without these, data above
/// 10000 collapses into `+Inf` and quantile estimates become meaningless.
///
/// Anything spanning more than ~1 decade uses [`log2_view`]. Two classes stay on
/// hand-picked boundaries: signed data (slumbot.hand_bb) and bounded data
/// (subgame.policy_deviation, mccfr.flush_duration_ms).
fn histogram_views() -> Vec<Box<dyn View>> {
    let views = [
        // Subgame CFR iterations per decision — observed 5k–500k on realtime variants.
        log2_view("rbp.subgame.iterations", 7, 20),
        // L1 distance between distributions — bounded 0..=2, so linear. The cliff of
        // interest is 0 (refined ≈ blueprint) vs 2 (fully disjoint).
        histogram_view(
            "rbp.subgame.policy_deviation",
            &[0.0, 0.05, 0.1, 0.2, 0.3, 0.5, 0.75, 1.0, 1.25, 1.5, 1.75, 2.0],
        ),
        // regret / pot — observed 0.5 to 2000+ in smokes; spans converged and diverged.
        log2_view("rbp.subgame.relative_regret", -7, 17),
        // bb/hand on Slumbot — signed (stack depth ~200 bb), so log2 doesn't apply.
        histogram_view(
            "rbp.slumbot.hand_bb",
            &[
        ...
        // Subgame decision wall-clock — capped at SubgameHyperParams::timeout_ms.
        log2_view("rbp.subgame.decision_ms", 0, 14),
        // K-means iteration wall-clock — typically <60s, full street caps ~30min.
        log2_view("rbp.kmeans.iteration_ms", 7, 21),
        // K-means phase wall-clock — init/iterate dominate, up to ~2h.
        log2_view("rbp.kmeans.phase_ms", 7, 23),
        // K-means cluster sizes — N varies by street (preflop ~1k, turn ~1.3M).
        log2_view("rbp.kmeans.cluster_size", 4, 24),
        // K-means per-cluster drift — descends from ~0.1 toward ~1e-5 over iterations.
        log2_view("rbp.kmeans.drift_dist", -20, 4),
        // Periodic DB-snapshot wall-clock — unimodal ~14s, observations 10-30s on a
        // 167K-infoset blueprint. Stay linear: log2 would compress the mode into one
        // bucket.
        histogram_view(
        ...
        // MCCFR tree size — Hold'em batches typically produce 1k-100k nodes.
        log2_view("rbp.mccfr.tree_size", 4, 20),
        // MCCFR infoset size — most are 1-5 nodes; the long tail is the diagnostic.
...
/// Explicit-bucket histogram view with power-of-two boundaries
/// `[2^min_exp, …, 2^max_exp]`, inclusive on both ends.
///
/// Each bucket pair covers a doubling, so a linear-y heatmap renders band
/// heights proportional to bucket widths. Exponents are signed so sub-unit
/// fractions (e.g. drift from 2^-20) are expressible.
