...
///
/// Encodes two abstraction indices plus their shared street into a single `u32`:
...
impl Pair {
    /// Constructs a pair from street and two abstraction indices.
    ///
    /// Indices are automatically sorted so `i < j`. When `i == j`,
    /// returns a degenerate pair with triangular index 0 (self-distance).
    ...
    /// Extracts the triangular index for array indexing.
