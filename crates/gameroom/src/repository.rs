...
impl HistoryRepository for Arc<Client> {
    async fn create_room(&self, room: &Room) -> Result<(), PgErr> {
        ...
        let sql =
            SQL.get_or_init(|| format!("INSERT INTO {} (id, stakes) VALUES ($1, $2)", rooms()));
    ...
    async fn create_hand(&self, hand: &Hand) -> Result<(), PgErr> {
        ...
        let sql = SQL.get_or_init(|| {
            format!(
                "INSERT INTO {} (id, room_id, board, pot, dealer) VALUES ($1, $2, $3, $4, $5)",
                hands()
            )
        });
    ...
    async fn update_visibility(
        ...
        let sql = SQL.get_or_init(|| {
            format!(
                "UPDATE {} SET visibility = $3 WHERE hand_id = $1 AND user_id = $2",
                players()
            )
        });
        self.execute(
            sql.as_str(),
            &[&hand.inner(), &user.inner(), &i16::from(visibility)],
        )
        .await
        .map(|_| ())
    }
    ...
    async fn get_hand(&self, hand: ID<Hand>) -> Result<Option<Hand>, PgErr> {
        ...
        self.query_opt(sql.as_str(), &[&hand.inner()])
            .await
            .map(|opt| {
                opt.map(|row| {
                    Hand::new(
    ...
    async fn get_players(&self, hand: ID<Hand>) -> Result<Vec<Participant>, PgErr> {
        ...
        self.query(sql.as_str(), &[&hand.inner()])
            .await
            .map(|rows| {
                rows.iter()
                    .map(|row| {
