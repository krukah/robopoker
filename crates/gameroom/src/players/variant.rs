//! Gameroom-side extensions to [`rbp_core::Variant`].
//!
//! The pure data shape of `Variant` lives in `rbp_core` so the WASM
...
use rbp_core::slumbot_opponent_username;
use rbp_core::slumbot_opponent_uuid;
use rbp_telemetry::KeyValue;

/// Gameroom-only methods on [`Variant`]. Implemented as an extension
/// trait because the type lives in `rbp_core` (which the WASM client
/// reads) and these methods need types from `rbp_auth`, `rbp_nlhe`,
/// `rbp_telemetry` that the client must not pull in.
