...
impl Size {
    ...
    ///
    /// Nlhe-specific adapter over [`rbp_translate`]: dispatches to the
    ...
    /// `None` if depth exceeds [`rbp_core::MAX_RAISE_REPEATS`].
    pub fn grid(street: Street, depth: usize) -> Option<Grid> {
        if depth > rbp_core::MAX_RAISE_REPEATS {
            ...
            match rbp_core::regime() {
                ...
                rbp_core::Regime::Slumbot => Some(Grid::Postflop(SLUMBOT_INDICES)),
            }
        }
    }

    /// Returns the raw RAISES-index slice for a given cell from the
    /// `PLURIBUS_INDICES` table-of-truth in `rbp_core`.
    ///
    ...
    pub fn raises(street: Street, depth: usize) -> Vec<Self> {
        ...
        match rbp_core::regime() {
            ...
            rbp_core::Regime::Slumbot => SLUMBOT_INDICES
                .iter()
                .map(|&i| {
                    let (n, d) = RAISES[i];
                    Self::SPR(n, d)
