...
pub trait Clusterable {
    ...
    fn cluster(&self,
        ...
        if !cfg!(feature = "kmeans-accel") {
            ...
            for _ in 0..t {
                ...
                let ref mut next = self.compute_next_kmeans(&working_centers);
                let ref mut last = working_centers;
            ...
            let mut ti_helpers: Vec<TriIneqBounds> = self
                ...
                .map(|nearest_neighbor| TriIneqBounds {
                    ...
                    lower_bounds: vec![0.0; self.kmeans_k()],
                    // "u(x)"
    ...
    fn compute_next_kmeans(&self, centers_start: &Vec<Histogram>) -> Vec<Histogram> where Self: Sync{
        ...
        for (point, (neighbor, distance)) in self
            .points()
            .par_iter()
            .map(|h| (h, self.nearest_neighbor(centers_start, h)))
            .collect::<Vec<_>>()
            .into_iter()
        {
    ...
    fn compute_next_kmeans_tri_ineq(
        ...
        let centroid_to_centroid_distances: Vec<Vec<f32>> = self
            ...
            .flat_map(|c| self.centers().iter().map(move |c_prime| (c, c_prime)))
            .collect::<Vec<_>>()
            .par_iter()
            .map(|(center1, center2)| self.distance(center1, center2)) // 1-D vector with length k^2
            .collect::<Vec<f32>>()
        ...
        use rayon::prelude::*;
        for (center_c_idx, center_c) in self.centers().iter().enumerate() {
            step_3_working_points
                ...
                .for_each(|(_point_i, (point_h, helper))| {
                    ...
                    let current_centroid_dist = if helper.stale_upper_bound {
                        let dist = self.distance(point_h, &self.centers()[helper.assigned_centroid_idx]);
                    ...
                    {
                        //  ... "Compute d(x,c)"
                        let dist_to_center_c = self.distance(point_h, center_c);
        ...
        for (centroid_i, points) in points_assigned_per_center.iter().enumerate() {
            ...
            if cfg!(feature = "kmeans-compute-nonfree-rms") {
                ...
                let old_centroid = &(self.centers()[centroid_i]);
                ...
                for point in points.iter() {
                    let distance_point_to_prior_centroid = self.distance(&old_centroid, &point);
    ...
    fn create_centroids_tri_ineq(&self) -> Vec<Neighbor> where Self:Sync {
        ...
        let nearest_neighbors: Vec<Neighbor> = self
            ...
            .map(|point| {
                ...
                let (index, initial_distance) = (0, self.distance(point, &(self.centers()[0])));
                // ... then continuing on for every other c
                let nearest_neighbor: Neighbor = self.centers().iter().enumerate().skip(1).fold(
                    ...
                    |acc, x_enumerated| {
                        ...
                        let next_center_distance = self.distance(point, next_center);
