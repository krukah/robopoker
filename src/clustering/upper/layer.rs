...
impl Layer {
    ...
    pub async fn raise(self) -> Self {
        let mut layer = Self {
            ...
            points: self.raise_points(),
            kmeans: self.raise_kmeans(),
        };
        layer.cluster(200);
        layer.upload().await;
        layer
    ...
    fn cluster(&mut self, iterations: usize) {
    ...
    #[rustfmt::skip]
    fn raise_points(&self) -> HashMap<Observation, (Histogram, Abstraction)> {
        Observation::all(self.street.prev())
            .into_iter()
            .map(|upper| (upper, (self.points.project(upper), Abstraction::default())))
            .collect()
    }
