...
pub struct Layer {
    ...
    observations: BTreeMap<NodeObservation, (Histogram, NodeAbstraction)>, // impl Projection
    abstractions: BTreeMap<NodeAbstraction, (Histogram, Histogram)>,
}
...
impl Layer {
    ...
    pub fn inner(&self) -> Self {
        let inner = Self {
            ...
            abstractions: self.inner_kmeans(),
            observations: self.inner_points(),
        };
        inner.kmeans()
    }
    ...
    pub async fn outer() -> Self {
        Self {
            ...
            abstractions: BTreeMap::default(),
            observations: Self::outer_points().await,
        }
    }
    ...
    fn kmeans(self) -> Self {
        assert!(self.abstractions.len() >= self.k());
    ...
    fn neighbor(&self, observation: &NodeObservation) -> NodeAbstraction {
        ...
        let ref histogram = self
            ...
            .0;
        for (centroid, (target, _)) in self.abstractions.iter() {
    ...
    fn recycle(&mut self) {
        for (_, (old, new)) in self.abstractions.iter_mut() {
            old.destroy();
    ...
    fn inner_points(&self) -> BTreeMap<NodeObservation, (Histogram, NodeAbstraction)> {
        ...
        NodeObservation::all(self.street.prev())
            ...
            .map(|inner| {
                (
                    inner,
                    (self.observations.project(inner), NodeAbstraction::default()),
                )
            })
            .collect()
    }
...
impl Layer {
    ...
    fn upload_centroid(&self) {
        ...
        let mut progress = Progress::new(self.observations.len(), self.observations.len() / 20);
